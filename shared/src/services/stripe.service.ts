...
export class StripeServerService extends StripeService {
	...
	constructor() {
		...
		if (!process.env.STRIPE_SECRET_KEY) {
			console.warn("⚠️ Stripe service not configured: missing STRIPE_SECRET_KEY. Payments are disabled.");
			return;
		}
	...
	async createPaymentIntent({
		...
		if (!this.stripe) {
			return { clientSecret: null, paymentIntentId: null, error: new ApiError("Stripe not configured", 500) };
		}
		try {
