import { collection, doc, getDocs, query, where, orderBy, serverTimestamp, runTransaction } from "firebase/firestore";
...
export class BookingService extends Service {
     ...
     */
    async createRegistration(tourId, travellersCount, notes) {
        if (!this.currentUid)
     ...
     */
    async adminCreateRegistration(tourId, customerId, travellersCount, notes) {
        return this.performRegistration(tourId, customerId, travellersCount, notes);
    }
    /**
     * Shared registration logic
     */
    async performRegistration(tourId, customerId, travellersCount, notes) {
        try {
            // Check for existing registration before starting transaction
            const existingRegQuery = query(collection(this.db, this.REGISTRATIONS_COLLECTION), where("tourId", "==", tourId), where("customerId", "==", customerId));
            ...
            return await runTransaction(this.db, async (transaction) => {
                ...
                if (!tourDoc.exists())
                    throw new ApiError("Pilgrimage journey not found", 404);
