...
const ToursMainPage = memo(() => {
	...
	const columns: ColumnDef<HighLevelTour, unknown>[] = [
		...
		},
		{
			...
			cell: (info) => (
				<HoverCard>
					<HoverCardTrigger className="hover:underline-offset-4 hover:underline cursor-pointer decoration-primary font-medium">
					...
					<HoverCardContent className="space-y-1">
						<img
							...
							alt="Tour Image"
							className="border border-border rounded-md object-cover h-32 w-full"
						/>
		...
		},
		{
			...
			accessorKey: "price",
			header: "Price (AED)",
			cell: (info) => <span className="font-semibold">{info.row.original.price.toLocaleString()}</span>
		},
		{
			id: "Status",
			accessorKey: "status",
		...
		},
		{
			...
			cell: ({ row }) => (
				<div className="text-xs">
					<div>S: {row.original.start_date || 'TBD'}</div>
					<div>E: {row.original.end_date || 'TBD'}</div>
				</div>
			)
		},
		{
