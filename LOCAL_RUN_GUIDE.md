...
- **Firebase**: You should have a Firebase project set up (the code is pre-configured for `toursandtravels-73c62`).
...
2. If not, copy it from the example:
   ...
   cp .env.example .env
   ```
   *(Note: I have already pre-filled the .env file with your project keys in this repository).*
...
### 💡 Troubleshooting Tip:
If you see a **"504 Outdated Optimize Dep"** error in the browser console, restart the server using the force flag to clear the cache:
```bash
npm run dev -- --force
```
...
---
*For more details on the architecture, see `FIREBASE_MIGRATION_GUIDE.md`.*
