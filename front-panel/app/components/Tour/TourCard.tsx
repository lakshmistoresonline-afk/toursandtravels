	...
	({
		...
		return (
			<Link to={"/tours/tour/" + tour.id} prefetch="intent" viewTransition className="group block h-full">
				<div className={cn(
					"h-full flex flex-col surface-card rounded-[2rem] overflow-hidden transition-all duration-500",
					"hover:border-[#d4af37]/40 hover:shadow-2xl hover:shadow-[#d4af37]/10 hover:-translate-y-1",
					className
				)}>
					{/* Image Section */}
					<div className="relative aspect-[4/3] overflow-hidden bg-black/60">
						{hasValidImage ? (
							...
							<div className="w-full h-full flex flex-col items-center justify-center gap-3 bg-[#d4af37]/5">
								<Compass className="h-10 w-10 text-[#d4af37] opacity-40" />
								<span className="text-[8px] font-bold uppercase tracking-[0.3em] text-[#d4af37]/40">Sacred Path</span>
							</div>
						)}
						...
						<div className="absolute top-5 left-5 z-20">
							<Badge className={cn(
								"px-3 py-1 text-[8px] font-bold uppercase tracking-widest border backdrop-blur-md rounded-full bg-black/60",
					...
					{/* Content Section */}
					<div className="p-8 flex flex-col flex-1 gap-6 bg-black/20">
						<div className="flex-1 space-y-4">
							<h3 className="font-serif text-xl leading-tight text-[#fdfcf0] group-hover:text-[#d4af37] transition-colors line-clamp-2">
								{tour.name}
							</h3>

							<div className="flex items-center gap-5 text-[#fdfcf0]/60 text-[9px] font-bold uppercase tracking-widest">
						...
						<div className="pt-6 border-t border-white/10 flex items-center justify-between">
							<div className="space-y-1">
								<p className="text-[8px] text-[#fdfcf0]/40 uppercase tracking-widest font-bold">Exchange</p>
								<p className="font-serif text-2xl text-[#d4af37]">
									{tour.price > 0 ? `₹${tour.price.toLocaleString()}` : "Inquire"}
								</p>
							</div>
							<div className="text-[8px] font-bold uppercase tracking-widest text-[#d4af37] group-hover:text-[#fdfcf0] transition-colors bg-[#d4af37]/10 px-4 py-2 rounded-full border border-[#d4af37]/20">
								View Journey
							</div>
						</div>
					</div>
